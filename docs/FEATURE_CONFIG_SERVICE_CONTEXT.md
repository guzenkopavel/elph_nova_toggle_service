...
Task 9 (Admin auth, RBAC, write path) is done: `src/modules/admin/auth.ts` is a Fastify preHandler that verifies the SSO JWT and enforces role-based access — `feature-toggle-viewer` for GET routes and `feature-toggle-editor` for mutations; `changedBy` is taken from the JWT `sub` claim. `src/modules/admin/service.ts` is `AdminRulesService`, which orchestrates the write sequence: validate registry key, validate `entry_json` against the manifest payload schema, detect ambiguous overlap, then `withTransaction(mutate + updateRevision + insertRevision)` followed by `invalidateCache`. `src/modules/admin/routes.ts` registers five routes under `/admin/api/rules` with zod body schemas; `ConflictError` maps to `409`, `ValidationError` to `400`, `NotFoundError` to `404`. `AuthResult.roles` was added to `src/modules/auth/token-verifier.ts`. 190 tests pass; typecheck is clean. Live smoke confirmed `401` without a token, public route regression-free, and health OK.
